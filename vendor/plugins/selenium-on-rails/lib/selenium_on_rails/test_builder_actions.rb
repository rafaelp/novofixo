...
module SeleniumOnRails::TestBuilderActions
  ...
  def setup options = {}
    ...
    [:fixtures, :clear_tables, :factories].each do |key|
      if (f = options[key])
        f = [f] unless f.is_a? Array
        opts[key] = f.join ','
      end
    end
    @cleared_tables = clear_tables opts[:clear_tables].to_s
    @loaded_fixtures = load_fixtures opts[:fixtures].to_s
    @loaded_factories = load_factories opts[:factories].to_s
    open opts
  end
